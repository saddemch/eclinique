...
export default function ModifierRendezVous() {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        ...
        setHeure(data.heure || "");
        setTypeConsultation(data.typeConsultation || "");
        setRappel(data.rappel || false);
        setMedecinId(data.medecinId?.toString() || "");
      } catch (err) {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <Button title={date || "Choisir une date"} onPress={() => setShowDatePicker(true)} />
      {showDatePicker && (
        <DateTimePicker
          value={new Date()}
          mode="date"
      ...
      <View style={styles.pickerWrapper}>
        <Picker selectedValue={heure} onValueChange={(value) => setHeure(value)}>
      ...
      <View style={styles.pickerWrapper}>
        <Picker selectedValue={typeConsultation} onValueChange={(value) => setTypeConsultation(value)}>
      ...
      <View style={styles.pickerWrapper}>
        <Picker selectedValue={medecinId} onValueChange={(value) => setMedecinId(value)}>
          {medecins.map((m) => (
            <Picker.Item
              key={m.id}
      ...
      <Button title="Modifier" onPress={submit} disabled={loading} />
